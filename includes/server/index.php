...
class CustomUploadHandler extends UploadHandler
{
    ...
    {
        global $db_pr;global $mysqli;global $userData;
        $file = parent::handle_file_upload($uploaded_file, $name, $size, $type, $error, $index, $content_range);

        if (empty($file->error)) {

	        $path     = addslashes($this->options['folder'] . "/" . $file->name);
	        $pathInfo = pathinfo($path);
	        $fileName = addslashes($file->name);
	        $sql      = "INSERT INTO {$db_pr}files (`title`,`path`,`size`,`extension`,`userID`,`catID`,`dateUploaded`) VALUES
                    ('{$fileName}','{$path}','{$file->size}','{$pathInfo['extension']}','{$this->options['idUser']}','{$this->options['folderID']}','" . date("Y-m-d H:i:s") . "')";
	        $res      = mysqli_query($mysqli,$sql);
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                if(isset($notify_upload) && isset($notify_email) && $notify_upload=="1"){
                    ...
                    $mailData['{%uploader%}'] = $userData['username'];
