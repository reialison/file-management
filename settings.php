...
if (isset($_SESSION["logged_in"]) && $_SESSION["logged_in"] != true) {
    ...
    if ($level == "admin") {
        ...
        $quota       = (!empty($_REQUEST["quota"])) ? strip_tags(str_replace("'", "`", $_REQUEST["quota"])) : 0;
        $filesize    = (!empty($_REQUEST["filesize"])) ? strip_tags(str_replace("'", "`", $_REQUEST["filesize"])) : 0;
        ...
        if (!empty($_REQUEST["submit_user"]) && $_REQUEST["submit_user"] == "yes") {
            $sql = "UPDATE {$db_pr}settings SET
              ...
              filesize='" . $filesize . "'
                ...
                WHERE id='1'";
            $result = mysqli_query($mysqli,$sql) or die($sql."Error occurred - tried  to save settings.");
            $msg = "<div class='loginMessage loginSuccess' >Settings saved!</div>";
            addLog($_SESSION["idUser"], "Updated script settings.");
            $showTab = 3;
        }
                ...
                <form action="settings.php" name="ff3" method="post" enctype="multipart/form-data"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4" id="backup_img">
                            ...
                            <img class="dnone" src="images/backup_loading.gif">
                        </div>
                    </div>
                    <div class="form-group">
